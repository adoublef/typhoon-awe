...
Deno.test("iam", async (test) => {
    await test.step("parse_profile()", async (test) => {
        await test.step("valid input data", (test) => {
            const input = { display: "adoublef", name: "Kristopher Rahim", };
            const output = parseProfile(input);
            assertExists(output.id);
        });
    });

    await test.step("parse_credentials()", async (test) => {
